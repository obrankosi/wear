...
public class gestionDocentesView extends JFrame implements IView, defaultLanguaje {
	...
	// asocio
	AdminListener bClearAlta = new AdminListener();
	IController gestionCClear_Alta = new gestionDocenteController();
	gestionCClear_Alta.setView(this);
	bClearAlta.associate(btnClear_alta, gestionCClear_Alta);
	...
	// asocio
	AdminListener bAddAlta = new AdminListener();
	IController gestionCAdd_Alta = new gestionDocenteController();
	gestionCAdd_Alta.setView(this);
	bAddAlta.associate(btnAdd_alta, gestionCAdd_Alta);
	...
	// asocio
	AdminListener bClearMod= new AdminListener();
	IController gestionCCleat_mod= new gestionDocenteController();
	gestionCCleat_mod.setView(this);
	bClearMod.associate(btnClear_modif, gestionCCleat_mod);

	
	JButton btnDel_modif = new JButton(DELETE);
	btnDel_modif.setBounds(109, 217, 89, 23);
	panelModif_doc.add(btnDel_modif);
	// asocio
	AdminListener bDeleteMod= new AdminListener();
	IController gestionCDelete_mod= new gestionDocenteController();
	gestionCDelete_mod.setView(this);
	bDeleteMod.associate(btnDel_modif, gestionCDelete_mod);


	JButton btnActualizar_modif = new JButton(MODIFY);
	btnActualizar_modif.setBounds(215, 217, 89, 23);
	panelModif_doc.add(btnActualizar_modif);
	// asocio
	AdminListener bUpdateMod = new AdminListener();
	IController gestionCUpdate_mod= new gestionDocenteController();
	gestionCUpdate_mod.setView(this);
	bUpdateMod.associate(btnActualizar_modif, gestionCUpdate_mod);
	...
	// asocio
	AdminListener bSearchAlta= new AdminListener();
	IController gestionCSearch_mod= new gestionDocenteController();
	gestionCSearch_mod.setView(this);
	bSearchAlta.associate(btnBuscar_modif, gestionCSearch_mod);
	...
	return !(this.getDireccionAlta().compareTo("") == 0)
		...
		&& !(this.getApellidoAlta().compareTo("") == 0)
		&& !(this.getNombreAlta().compareTo("") == 0)
		&& !(this.getFechaAlta().compareTo("") == 0);

    }

    public void deleteViewFieldsMod() {
	// TODO Auto-generated method stub
	this.setTelefonoModif("");
	this.setNombreModif("");
	this.setFechaModif("");
	this.setDniModif("");
	this.setDireccionModif("");
	this.setApellidoModif("");
	this.setTelefonoAlta("");
	this.setNombreAlta("");
	this.setFechaAlta("");
	this.setDniAlta("");
	this.setDireccionAlta("");
	this.setApellidoAlta("");
